	...
	def _build_graph(self):
		...
		with tf.variable_scope(self.scope, reuse=tf.AUTO_REUSE):
			...
			if self.mode != tf.estimator.ModeKeys.PREDICT:
				...
				labels, s0_ids, s1_ids = self.iterator.get_next()
				
				self.x1 = tf.nn.embedding_lookup(self.word_embedding, s0_ids)
				self.x2 = tf.nn.embedding_lookup(self.word_embedding, s1_ids)
				...
				self.x2 = tf.nn.embedding_lookup(self.word_embedding, self.s1_ids)
