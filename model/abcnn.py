...
class ABCNN(BCNN):
	...
	def _attention_matrix(self, x1, x2):
		with tf.name_scope('attention_matrix'):
			...
			# x1_tile: (b, s1, s2), x2_tile: (b, s2, s1)
	...
	def _attention_feature_map(self, x, s, a):
		_, _, dim, _ = x.get_shape().as_list()
		with tf.name_scope('attention_feature_map'):
			w = tf.get_variable(
				name='W',
				shape=(s, dim),
			...
			w = tf.tile(tf.expand_dims(w, axis=0), [self.config.batch_size, 1, 1])
			f = tf.matmul(a, w)
	...
	def _attention_pooling_weights(self, x1, x2):
		...
		# attention_w1: (b, s1+w-1), attention_w2: (b, s2+w-1)
	...
	def _cnn_block(self, x1, x2, filter_size, num_layers):
		s1, s2 = self.config.s0_max_len, self.config.s1_max_len
		out1, out2 = [], []
		...
		for i in range(1):
			with tf.variable_scope("window-{}/cnn-block-{}".format(filter_size, i+1)):
				...
				padded1 = self._wide_conv_padding(x=x1, padding_size=filter_size)
				padded2 = self._wide_conv_padding(x=x2, padding_size=filter_size)
